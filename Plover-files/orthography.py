...
word_list_file_name = os.path.join(ASSETS_DIR, 'france_french_words.txt')
WORDS = dict()
try:
...
RULES = [
    ...
    # == +s ==
    # AZIZ establish + s = establishes (sibilant pluralization)
    (re.compile(r'^((?: )*(?:s|x|z)) \^ s$', re.I),
        r'\1s'),
    (re.compile(r'^((?: )*(?:i))(er) \^ s$', re.I),
        r'\1z'),
    (re.compile(r'^((?: )*(?:i))(er) \^ é', re.I),
        r'\1é'),
    (re.compile(r'^((?: )*(?:i))(er) \^ és', re.I),
        r'\1és'),
    (re.compile(r'^((?: )*(?:i))(er) \^ ée$', re.I),
        r'\1ée'),
    (re.compile(r'^((?: )*(?:i))(er) \^ ées$', re.I),
        r'\1ées'),
    (re.compile(r'^((?: )+)(er) \^ ait$', re.I),
        r'\1ait'),
    (re.compile(r'^((?: )+)(er) \^ aient$', re.I),
        r'\1aient'),
